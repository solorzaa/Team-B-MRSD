...
//	usleep(100);  
        ...
        char valuemxH=Read_I2C_Byte(HMC5883L_I2C_ADDRESS,HMC5883L_X_MSB);
	char valuemxL=Read_I2C_Byte(HMC5883L_I2C_ADDRESS,HMC5883L_X_LSB);

	char valuemzH=Read_I2C_Byte(HMC5883L_I2C_ADDRESS,HMC5883L_Z_MSB);// read data from the registers
	char valuemzL=Read_I2C_Byte(HMC5883L_I2C_ADDRESS,HMC5883L_Z_LSB);
	...
	char valuemyL=Read_I2C_Byte(HMC5883L_I2C_ADDRESS,HMC5883L_Y_LSB);	
	 ...
	 signed int valuemz=valuemzH<<8|valuemzL;
...
void myI2C::compass_init(int gain, unsigned char DEVICE_ADDR){
  ...
  cout<<"Gain updated to  = ";  cout<<compass_gain_fact;   cout<<" mG/bit" <<endl;
...
void myI2C::compass_offset_calibration(int select, unsigned char DEVICE_ADDR){
 ...
 // Configuring the Control register for Positive Bais mode
  ...
  //Wire.beginTransmission(compass_address);
  i2cSetAddress(DEVICE_ADDR);
  Send_I2C_Byte(HMC5883L_I2C_ADDRESS,HMC5883L_CONFIG_REG_A,0b01110001);	
  //Wire.write(0x00);
...
//	cout <<"***********************************************"<<endl;
  ...
  Send_I2C_Byte(HMC5883L_I2C_ADDRESS,HMC5883L_CONFIG_REG_A,0x72);
  ...
  Send_I2C_Byte(HMC5883L_I2C_ADDRESS,HMC5883L_CONFIG_REG_A,0x78);
  /*
  A A                        DO2 DO1 DO0      Sample Rate [Hz]      MS1 MS0    Measurment Mode
   0 0 = No Average            0   0   0   =   0.75                   0   0   = Normal  
   0 1 = 2 Sample average      0   0   1   =   1.5                    0   1   = Positive Bias
  ...
  // *****************************************************************************************
    ...
    */
    ...
    // End Debug code
      ...
      compass_read_XYZdata(DEVICE_ADDR);
      //cout << "t" << t <<endl;
      compass_x_scalled=compass_x*compass_gain_fact*compass_x_gainError;
      compass_y_scalled=compass_y*compass_gain_fact*compass_y_gainError;
      compass_z_scalled=compass_z*compass_gain_fact*compass_z_gainError;
      
      if (compass_debug == 1){  //------------------ Debug Data
        cout<< "  cx" << compass_x_scalled ;
       
        cout<< "  cy" << compass_y_scalled ;

        cout<< "  cz" << compass_z_scalled <<endl;
        }//--------------------------------- End Debug Data
