...
//Kalman Filter
void runKalman(double absDeltaXe, double absDeltaYe, double absDeltaTe);
...
int main(int argc, char *argv[])
{
	...
	q << 10.3495, -2.3251, 0.9759,  //Covariance of the model
             -2.3251, 3.3519, -0.2221,
             0.9759, -0.2221, 0.0921;
	cout << "initial q: " << q << endl;
	Q.bottomRightCorner(3, 3) = q;                  //put covariance into Q
...
/*	if(leicaConnected) {
	...
	//absoluteX += absDeltaX;
	//absoluteY += absDeltaY;
	runKalman(absDeltaX, absDeltaY, deltaTheta);
...
{
	//Update observations
        z << absDeltaXe, absDeltaYe, absDeltaTe;
        z = z/diffTime;
	cout << "z = " << endl << z << endl;
        //Constant velocity prediction filtering
        F.topRightCorner(3,3) = Eigen::Matrix3d::Identity() * diffTime;
        //cout << "F = " << F << endl;
