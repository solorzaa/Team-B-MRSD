...
float thetaCorrection = 0.915;//0.937;
int leftWheelCode = 2;
...
// Time Shifting for Leica
vector<double> veloSet;
...
int main(int argc, char *argv[])
{
	...
	while(inProgress = desiredPathXY(currentTime, xGoal, yGoal, thetaGoal)){
		...
		// Time Shifting Code
		veloSet.push_back(velocityCommands[0]);
...
bool calibrateLeica(char* dataString, float leicaTimeStamp)
{
	...
	//Shifting
	double dataTime = testData[0];
	gettimeofday(&cTShift, NULL);
	absoluteTimeShift = (cTShift.tv_usec/1.E6)-dataTime;
...
void timeShift() {
	...
	// Check to see if last leica time was reasonable
	gettimeofday(&cTShift, NULL);
	...
	// Project the point forward over the discete points
	for(int i = projectingStart+1; i<veloSet.size()-1; i++) {
		timeToNextDiscrete = timeSet[i+1]-timeSet[i];
		v = veloSet[i];
		w = anguSet[i];
		theta = thetaSet[i];
		changeX = -(v/w)*(1-cos(w*timeToNextDiscrete));
	...
	// Project the point forward up until the current point
	timeToCurrent = cTime-timeSet[fifoSize-1];
	v = veloSet[fifoSize-1];
	...
	cout << "Leica Projected X: " << shiftedLocation[0] << endl;
	cout << "Leica Projected Y: " << shiftedLocation[1] << endl;
