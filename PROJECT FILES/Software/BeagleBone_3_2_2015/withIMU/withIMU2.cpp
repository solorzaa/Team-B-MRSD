...
int main(int argc, char *argv[])
{
	...
	//Initialization
	myI2C *i2cptr = new myI2C();
	//calibrating gyroscope and magnetometer
	usleep(10);
	cout << "Calibrating IMU..." << endl;
	//IMUoffsetGyro=i2cptr->Itg3200calibrate();
	//cout <<"offset is:" << IMUoffsetGyro <<endl;
	// Initialization 
	while(true) {
		try {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////	
	...
	while(inProgress = desiredPathXY(currentTime, xGoal, yGoal, thetaGoal)){
		...
		//if(imuLag%3==0) {
		//	gyroValue = getGyroZ(i2cptr);
		//	if(getHeading(i2cptr,headingValue))
		//		cout << "Heading in Radians: " << headingValue << endl;
		//}
		//if(imuLag%2==0) {
		//	try { 
		//		i2cptr->Send_I2C_Byte(HMC5883L_I2C_ADDRESS,HMC5883L_MODE_REG,0X00);
		...
		//	catch(...) {
		//		imuLag++;
		//		perror("IMU - Failed to Send Mode");
		//	}
		//}
		//imuLag++;
...
float getGyroZ(myI2C *i2cptr) {
	...
	//a		}
	//usleep(100000);
	//a	gyroz=gyroz/(float)average;
	//offset obtained from callibration step
	gyroz = angle;
	gyroz=gyroz+IMUoffsetGyro;
	cout << "Gyro: " << gyroz << endl;

	return gyroz;//*M_PI/180;
}
//////////////////////////////////////////
